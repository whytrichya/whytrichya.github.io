  ...
  <!-- Eksekusi Form Login -->
      <?php 
        if(isset($_POST['submit'])){
          ...
          $cek = mysqli_query($koneksi, "SELECT * FROM user WHERE username = '$user' AND password_user = '$pass'");
          if(mysqli_num_rows($cek) > 0){
              $d = mysqli_fetch_array($cek);
              $login_user = $d['username'];
              session_start();
              $_SESSION['login_user'] = $login_user;

              echo "<script>alert('Anda telah berhasil login!');</script>";
